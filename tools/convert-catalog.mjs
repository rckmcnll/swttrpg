...
// 4b. Species (all lines — species are universal to any character; carries OggDude
//     SpeciesKey + base stats so the sheet/exporter need not guess from the name)
const species = {};
for(const f of fs.readdirSync(path.join(DATA,'Species')).filter(f => f.endsWith('.xml'))){
  const sp = readXML(path.join(DATA,'Species',f));
...
// 7. Equipment (weapons / armor / gear) — all game lines (universal to any character)
const RANGE = { wrEngaged:'Engaged', wrShort:'Short', wrMedium:'Medium', wrLong:'Long', wrExtreme:'Extreme' };
const desc = {};
try { for(const d of kids(readXML(path.join(DATA,'ItemDescriptors.xml')),'ItemDescriptor')) desc[ktext(d,'Key')] = { name: ktext(d,'Name'), qual: ktext(d,'QualDesc'), boolean: ktext(d,'IsBoolean') === 'true' }; }
catch { warnings.push('ItemDescriptors.xml missing; weapon qualities shown as keys'); }
function qualityStr(qkey, count){ const d = desc[qkey]; let s = d ? (d.qual || (d.name || qkey).replace(/ Quality$/i,'')) : qkey; if(s.indexOf('{0}') >= 0) s = s.replace('{0}', count != null ? count : '').trim(); else if(count != null && !(d && d.boolean)) s = s + ' ' + count; return s; }
function weaponQualities(w){ const out = []; const qn = kid(w,'Qualities'); if(qn) kids(qn,'Quality').forEach(q => { const k = ktext(q,'Key'); const c = kid(q,'Count') ? Number(ktext(q,'Count')) : null; out.push(qualityStr(k, c)); }); return out; }
const equipment = { weapons:{}, armor:{}, gear:{} };
for(const w of kids(readXML(path.join(DATA,'Weapons.xml')),'Weapon')){
  const dmg = ktext(w,'Damage') || (kid(w,'DamageAdd') ? '+' + ktext(w,'DamageAdd') : '');
  const hp = Number(ktext(w,'HP') || 0);
  const special = weaponQualities(w).concat(hp > 0 ? ['HP ' + hp] : []).join(', ');
  equipment.weapons[ktext(w,'Key')] = { key: ktext(w,'Key'), name: ktext(w,'Name'),
    skill: skillName[ktext(w,'SkillKey')] || ktext(w,'SkillKey') || '', dam: dmg, crit: ktext(w,'Crit'),
    range: RANGE[ktext(w,'RangeValue')] || '', enc: ktext(w,'Encumbrance'), special, price: ktext(w,'Price'), rarity: ktext(w,'Rarity') };
}
for(const a of kids(readXML(path.join(DATA,'Armor.xml')),'Armor')){
  equipment.armor[ktext(a,'Key')] = { key: ktext(a,'Key'), name: ktext(a,'Name'), defense: ktext(a,'Defense'),
    soak: ktext(a,'Soak'), enc: ktext(a,'Encumbrance'), hp: ktext(a,'HP'), special: '', price: ktext(a,'Price'), rarity: ktext(a,'Rarity') };
}
for(const g of kids(readXML(path.join(DATA,'Gear.xml')),'Gear')){
  equipment.gear[ktext(g,'Key')] = { key: ktext(g,'Key'), name: ktext(g,'Name'), enc: ktext(g,'Encumbrance'),
    notes: ktext(g,'Short') || '', price: ktext(g,'Price'), rarity: ktext(g,'Rarity') };
}
...
const catalog = {
  ...
  equipment: { weapons: sortKeys(equipment.weapons), armor: sortKeys(equipment.armor), gear: sortKeys(equipment.gear) }
};
...
console.log(`equipment:         ${Object.keys(equipment.weapons).length} weapons, ${Object.keys(equipment.armor).length} armor, ${Object.keys(equipment.gear).length} gear`);
